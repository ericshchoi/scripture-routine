...
function buildStairs(){
 ...
 for(let i=0;i<count;i++){
   let s=document.createElement("div");s.className="step";s.dataset.i=i;
   let x=(i/(count-1))*78, y=(i/(count-1))*78;
   s.style.left=`${x}%`;s.style.bottom=`${y}%`;s.style.width=`${27-i*.65}%`;
   host.appendChild(s);
 }
}
function renderJourney(p){
 const child=$("#prayerChild"), bubble=$("#journeyBubble");
 const x=12+p*.68, y=15+p*.63;
 child.style.left=`${x}%`; child.style.bottom=`${y}%`;
 bubble.textContent=`${pctText()}%`;
 $$(".step").forEach((s,i)=>s.classList.toggle("active",(i/12)*100<=p));
}
